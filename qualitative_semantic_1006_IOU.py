...
def calculate_mae_mse(predictions, ground_truths):
...
def test_simple(args):
    ...
    # path= "./data/WoodScape_ICCV19/test1004/test_image/*"  # 1003 선정 테스트데이터, 정상없음
    
    # path= "./data/WoodScape_ICCV19/data_last1014/*"  # 1003 선정 테스트데이터, 정상없음
    path = "./data/WoodScape_ICCV19/test_last_origin1012/test_image/*" # 정상 포함 테스트 데이터
    ...
    for idx, image_path in enumerate(image_paths):
        ...
        # 오염도 레벨을 리스트에 저장
        pred_levels.append(level_pred)
        gt_levels.append(level_gt)
        ...
        # print(f"Class 1 Accuracy: {accuracy_class_1:.4f}")
        # print(f"Class 2 Accuracy: {accuracy_class_2:.4f}")
        ...
        # print(f"Cumulative Accuracy for Class 1 after {idx + 1} images: {cumulative_accuracy_class_1:.4f}")
        # print(f"Cumulative Accuracy for Class 2 after {idx + 1} images: {cumulative_accuracy_class_2:.4f}")
        # print(f"Cumulative Accuracy for Class 3 after {idx + 1} images: {cumulative_accuracy_class_3:.4f}")
    ...
    # 전체 이미지 처리 후 MAE와 MSE 계산
    mae, mse = calculate_mae_mse(pred_levels, gt_levels)
